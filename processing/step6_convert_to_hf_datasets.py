...
INPUT_CSVs = {
    '5a': os.path.join(DATA_DIR, "5a-generate-union-dataset", "union_dataset.csv"),
...
union_hf: Dataset = Dataset.from_pandas(
    df=union_df,
    preserve_index=True
)
union_hf = union_hf.cast(
    features=Features({
...
long_df: DataFrame = pd.read_csv(
    filepath_or_buffer=path,
    index_col=0
)

long_hf: Dataset = Dataset.from_pandas(
    df=long_df,
    preserve_index=False,
)
long_hf = long_hf.cast(
    features=Features({
